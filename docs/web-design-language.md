...
| Dividers            | Region boundaries use `border-border`; content dividers use `border-subtle`. Inside the workbench `--border` is an 8% foreground hairline. | `globals.css`, including the workbench-local subtle-divider derivation                                         |
