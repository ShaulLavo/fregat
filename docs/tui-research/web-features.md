...
### 2.2 Editor commands (`keymap/editor-commands.ts`, dispatched into `@singapore-editor/core` editor)
...
- `Wallpaper` selects a source per light/dark mode through `workbench.wallpaper`. Library stills render on app and compositor backdrops; transparent windows render none. `WebWallpaper` retains Desktop still/video behavior on the app backdrop.
...
Appearance (window): `workbench.colorTheme`, `workbench.palette`, `workbench.density`, `workbench.surface.opacity|contentOpacity|blur|saturation`, `workbench.tree.indentGuides`. Editor (window): `editor.fontFamily|fontSize|lineHeight|tabSize|diff.viewMode|minimap.enabled|guides.indentation|syntaxHighlighting.enabled|decode.mode`. Terminal (window): `terminal.integrated.fontSize|scrollback|cursorBlinking`. Search (window): `search.defaultMatchMode|caseSensitive|wholeWord|maxResults|maxResultFiles|quickOpenLimit`. Wallpaper (application): `workbench.wallpaper`. Chat (application): `chat.defaultRuntimeMode|defaultInteractionMode`. Logs (window): `logs.defaultTimeRange|slowThresholdMs`. Window (machine): `window.transparency`. Files (window): `files.autoSave|autoSaveDelay|showHidden`. Language servers (machine): `lsp.experimental.tyForPython|idleTimeoutMs|downloadRuntimes|servers|languageServers|semanticTokens.enabled|delta|servers`. Providers/Models/Keys (application): `providers.instances`, `models.hidden`, `models.order`, `keybindings.overrides`.
