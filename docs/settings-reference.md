...
| `editor.inputRoute`                           | `"textarea"`      | application | How typed text reaches the editor. EditContext (Chromium) receives IME, autocorrect and dictation edits with their exact ranges instead of reading them back out of a hidden textarea; other browsers always use the textarea. _(restart)_                       |
