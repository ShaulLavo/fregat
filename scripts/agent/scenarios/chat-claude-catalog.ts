...
export const chatClaudeCatalog: Scenario = {
  ...
  async run(page, { step }) {
    ...
    try {
      ...
      await selectors.modelOptionSwitch(page, 'Fast mode').waitFor()
      ...
      await selectors.modelOptionSwitch(page, 'Fast mode').click()
      strictEqual(
        await selectors.modelOptionSwitch(page, 'Fast mode').getAttribute('aria-checked'),
        'true',
      )
      await step('opus-fast-mode-on')
      await selectors.modelOptionChoice(page, 'Reasoning', 'Ultracode').hover()
      await page.getByText('Extra high effort plus multi-agent workflow orchestration').waitFor()
      await step('level-description-tooltip')
      await selectors.modelOptionSwitch(page, 'Fast mode').click()
      ...
      await selectors.modelOptions(page).click()
      await selectors.modelOptionChoice(page, 'Context window', '1M').waitFor()
      strictEqual(await selectors.modelOptionSwitch(page, 'Fast mode').count(), 0)
      await settleAnimations(selectors.popupMenu(page))
