...
- Scrollbars are one base-layer rule: thin, invisible at rest, shown on hover or focus-within. A call site never styles one. `no-scrollbar` is for horizontal strips (census `scrollIdiom`); `scroll-gutter` goes on a vertical scroller that crosses the overflow threshold while watched; `scroll-fade` is on by default in `VirtualList`, `ToolPane` bodies and `CommandList`; `scroll-pinned` hides the thumb while a view follows its tail. A `ToolPane` whose body hosts its own scroller or a terminal passes `scroll={false}`.
...
- A key is drawn by `Kbd` (`bg-current/10`, no border) and nowhere else (census `kbdSpelling`). A menu shows its shortcut as a trailing chip; a tooltip on an icon-only trigger that dispatches a bound command shows it too, read through `useCommandShortcut`. The `*Shortcut` slots stay generic trailing metadata and wrap a `Kbd` only when they carry a key.
