...
export function PendingChordIndicator({ pending }: { readonly pending: PendingChordLabel | null }) {
  return (
    <output aria-atomic='true' aria-live='polite'>
      {pending ? (
        <span className='bg-popover-solid border-border text-foreground pointer-events-none fixed bottom-4 left-4 z-50 flex max-w-[calc(100vw-2rem)] flex-wrap items-center gap-x-2 rounded-lg border px-3 py-2 text-xs shadow-lg'>
