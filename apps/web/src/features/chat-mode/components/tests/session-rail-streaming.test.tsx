...
test.for([false, true])(
  'detail deltas do not render rail/palette with bulk selection %s',
  async (selected, { client, server }) => {
    ...
    const palette = renderHookWithProviders(
      () => {
        paletteRenders += 1
        return useSessions()
      },
    ...
    try {
      ...
      if (selected)
        await act(async () => {
          useSessionMultiSelectStore.setState({
            refs: harness.sessionIds.map((sessionId) => ({
      ...
      expect(
        palette.result.current.sessions.find((session) => session.id === sessionId)?.title,
      ).toBe('Renamed')
