...
test('lists SSH aliases and tailnet peers together and saves the chosen tailnet target', async () => {
  ...
  expect(await screen.findByRole('option', { name: 'my-ssh-alias' })).toBeVisible()
  const tailnet = within(screen.getByRole('group', { name: 'Tailnet machines' }))
  expect(await tailnet.findByRole('option', { name: /^devbox / })).toBeVisible()
  const offline = tailnet.getByRole('option', { name: /^sleeping / })
  ...
  expect(tailnet.queryByRole('option', { name: /^sleeping / })).toBeNull()
  await userEvent.click(tailnet.getByRole('option', { name: /^devbox / }))
...
test('refreshing an unavailable tailnet shows progress while SSH aliases remain usable', async () => {
  ...
  expect(screen.getByRole('option', { name: 'available-ssh' })).toBeEnabled()
  await act(async () => pending.resolve(await tailnetStatusCommand({ peers: [tailnetPeer()] })()))
  expect(await screen.findByRole('option', { name: /^devbox / })).toBeVisible()
})
