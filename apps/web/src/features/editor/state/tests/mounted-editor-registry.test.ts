...
test('does not publish a false mounted-editor transition during a StrictMode replay', async () => {
  ...
  const unregister = registry.register(filesystemPath('/repo/a.ts'))
  unregister()
  registry.register(filesystemPath('/repo/a.ts'))
  await Promise.resolve()

  expect(listener.mock.calls).toEqual([['/repo/a.ts', true]])
  expect(registry.has(filesystemPath('/repo/a.ts'))).toBe(true)
})
...
test('publishes a real last-owner unmount after the replay window', async () => {
  ...
  const first = registry.register(filesystemPath('/repo/a.ts'))
  const second = registry.register(filesystemPath('/repo/a.ts'))
  first()
  second()
  await Promise.resolve()
  ...
  expect(registry.has(filesystemPath('/repo/a.ts'))).toBe(false)
})
