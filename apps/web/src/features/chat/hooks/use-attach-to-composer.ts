...
export function useAttachToComposer() {
  ...
  const reveal = (source: string, detail: Record<string, unknown>) => {
  ...
  const attachTerminalContext = (selection: TerminalContextSelection | null) => {
  ...
  const attachText = (source: string, text: string) => {
   ...
   */
  const attachTextToNewChat = async (source: string, text: string, rootPath: string) => {
