...
import { useAttachToComposer } from './use-attach-to-composer'
...
export function useFixWithAgentBinding() {
  ...
  useEffect(
    () => bindFixWithAgent((prompt) => attachTextToNewChat('fix-with-agent', prompt, rootPath)),
    [attachTextToNewChat, rootPath],
  )
}
