...
test('a delayed Git diff cannot replace a newer file destination', async ({ client, server }) => {
  ...
  expect(parseDiffDocumentId(harness.workspace.getState().selectedFilePath)?.path).toBe('repo/a.ts')
...
test('completed rename and discard survive a newer file destination', async ({
  ...
  const renamed = commands.renameLiveEditorDocument('repo/a.ts', 'repo/renamed.ts')
  ...
  expect(editorTabPaths(harness.workspace)).toEqual(['repo/renamed.ts', 'repo/b.ts'])
  await unlink(path.join(server.root, 'repo/b.ts'))
  const discarded = commands.discardLiveEditorDocument('repo/b.ts')
  expect(await navigation.openFile({ owner: harness.workspace, path: 'repo/c.ts' })).toEqual({
    status: 'applied',
  })
...
test('stale search, diff and project commands cannot supersede a pending file destination', async ({
  ...
  const next = navigation.openFile({ owner: harness.workspace, path: 'repo/b.ts' })
...
test('closing the addressed editor selects its conflict successor without reopening the closed file on a panel edit', async ({
  ...
  await waitForNavigation(navigation)
  const conflict = conflictDiffDocumentId('close-successor')
  ...
  const tab = harness.workspace
    .getState()
    .workbenchPanels.editorTabs.find((item) => item.path === 'repo/a.ts')
  if (!tab) return expect.unreachable('addressed tab is missing')
  expect((await navigation.editorCommands(harness.workspace).closeTab(tab.id)).status).toBe(
    'applied',
  )
  expect(harness.workspace.getState().selectedFilePath).toBe(conflict)
  expect(editorTabPaths(harness.workspace)).toEqual([conflict])
