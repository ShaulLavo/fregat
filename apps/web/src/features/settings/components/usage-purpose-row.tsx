...
import { formatModelCost, usagePurposeLabel } from '@/features/settings/utils/usage'
...
export function UsagePurposeRow({ row }: { readonly row: ProviderUsagePurposeRow }) {
  return (
    <li className='flex items-baseline gap-3 text-xs' data-usage-purpose={row.purpose}>
      ...
      <span className='min-w-16 shrink-0 text-right tabular-nums'>{formatModelCost(row)}</span>
    </li>
  )
}
