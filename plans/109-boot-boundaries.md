...
It does not cover the instrument that measures first load (Plan 106 owns it),
...
(Plan 119), or the scoped error boundary and the
`<Activity>`/visibility rule it leans on
(Plan 127, [Plan 128](128-react-19-patterns.md)).
...
Plan 106 established that this is not a bundler problem:
...
The original dependency statement — "It runs after 107 and
...
Plan 106 Phase 3 removed the Phosphor icon **font**, `@phosphor-icons/web`,
...
### Phase 1 result (2026-09-25, lane L4)
...
1. **Error boundary first (D7).** Before any `lazy()` ships, a scoped boundary renders a retry in
   ...
   Plan 127 lands the scoped boundary first, consume it;
...
### Phase 4 result (2026-09-25, lane L4)
...
- Each boundary is checked in the browser for the interaction it defers, with
  Plan 119's CLI, against a running dev server. There is no
