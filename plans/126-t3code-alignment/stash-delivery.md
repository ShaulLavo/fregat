...
Focused tests cover one-write prompt/context transfer, swap, quota failure, pending work, environment isolation and real composer keyboard/menu behavior. `chat-stash-context` covers image-only stash, image/file payload swap, reload and native file consumption. The historical live proof follows below. Review comments, preview annotations and citations remain INTERACTION-09 work; this unit preserves the existing terminal context domain.
