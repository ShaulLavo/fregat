...
| (deleted) row-height audit                                                        | Removed with E050's typography options (plan 130, D3): the editor owns its font and row height, so nothing is left to disagree.                                                                                                                  |
...
`bun run agent:browser scenario editor-typography` to prove the editor still re-measures on a font change.
