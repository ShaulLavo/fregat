...
export function barTabClassName(active: boolean, className?: string) {
  return cn(
    'flex shrink-0 items-center gap-(--density-control-gap) border-x border-b px-(--density-control-padding-x) text-xs whitespace-nowrap',
    active
      ? 'border-x-border bg-content-well text-foreground border-b-transparent'
      : 'border-x-transparent border-b-border text-muted-foreground hover:bg-row-hover active:bg-row-active hover:text-foreground',
    className,
  )
}
