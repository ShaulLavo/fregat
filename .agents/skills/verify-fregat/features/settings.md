...
`scenario prefetch-settings` searches for "prefetch", checks the master switch and each surface's switch indented under it, turns the master off (surfaces disabled and off) and back on.
