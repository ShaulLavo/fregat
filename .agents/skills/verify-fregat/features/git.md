...
`scenario git-merge-request` registers a fixture on `feature/merge`, one commit ahead of its pushed upstream, whose `origin` is `git@gitlab.com:fregat/fixture.git`; a repo-local `core.sshCommand` (`createSshRemote` in `scripts/agent/fake-forge.ts`) serves it from a local bare repository, because `insteadOf` would change what `git remote -v` prints. `scripts/agent/fixtures/fake-glab.mjs` is first on the throwaway server's PATH. The header must offer `Push and open merge request`; clicking it must push the commit to the bare repository, call `glab mr create --source-branch feature/merge`, show the `#5` link and toast `Pushed and opened`. Other forges are covered by `apps/server/src/git/tests/forges.test.ts` at the CLI/HTTP boundary.

`scenario git-clone-publish` runs `Clone repository…` from the palette into a new folder beside a fixture workspace and must land in the cloned workspace with its file on disk. It then opens a fixture repository with no remote, uses the Git pane's `Publish repository` with GitHub, `acme/app` and HTTPS (a fake `gh` answers `repo create`; `url.<bare>.insteadOf` sends the push to a local bare repository), and requires the `Repository published` toast, the branch in the bare repository, and Fetch back in the pane.
