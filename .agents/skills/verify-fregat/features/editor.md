...
`scenario editor-storage-maintenance` builds a disposable workspace, replaces its file three times and types and backspaces a word forty times, then waits for the editor's quiet-time storage pass (Editor E006). It reads the pass from the `editorPerfTrace` diagnostics (`buffer.reclamation`): tombstones compacted must be above zero with none `unverified`, and `inspection.json` keeps the detail. It then undoes back to the second replacement and redoes, across the compacted tree.
...
`scenario prefetch-first-paint` opens files and diffs from every press surface in a fixture repository (quick open cold and warm, keyboard next tab, tree rows with no hover and after 1.5 s and 7 s hovers, git diffs first and revisited) and prints, per press, milliseconds from the input event to the target's first text, syntax colour and markdown preview frame; `inspection.json` holds the table. The same opens log `prefetch` (`hit`, `partial`, `miss`, `live`) and `firstPaint` on `editor.command.select_file`, `editor.command.select_tab` and `editor.command.open_diff`, and each intent logs one `prefetch.intent` (`surface`, `outcome`, `leadMs`, `prepareMs`, `workerMs`). Run it before and after any prefetch change.
