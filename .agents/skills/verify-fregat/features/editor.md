...
`scenario prefetch-first-paint` opens files and diffs from every press surface in a fixture repository (quick open cold and warm, keyboard next tab, tree rows with no hover and after 1.5 s and 7 s hovers, git diffs first and revisited) and prints, per press, milliseconds from the input event to the target's first text, syntax colour and markdown preview frame; `inspection.json` holds the table. The same opens log `prefetch` (`hit`, `partial`, `miss`, `live`) and `firstPaint` on `editor.command.select_file`, `editor.command.select_tab` and `editor.command.open_diff`. Run it before and after any prefetch change.
