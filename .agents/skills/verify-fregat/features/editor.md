...
`scenario editor-edit-context-input` switches `editor.inputRoute` (restored afterwards), reloads, and in a fixture workspace types, composes through CDP IME commands and applies a correction over the first word. The EditContext route must produce `Hello日本` in a `div`; the textarea control records `hello日本Hello`, the known correction bug of that route. Editors are reused across tabs, so a route change needs the reload.
