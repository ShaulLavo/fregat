...
- `chat-claude-catalog`: the Claude list read from the running CLI — current models with Fable 5.1 first, retired ones under the collapsible Legacy row, Opus 5.5 options with Fast Mode and 1M, Fable 5.1 without Fast. Restores the project default model.
...
- `settings-usage`: Settings › Usage — the real `/providers/usage/history` read (asserted 200, screenshot `real-read`), then a fixed month: headline with the unpriced-token note, 30 day slots with a bar tooltip, one row per model and purpose, and the price editor for the unpriced model. Writes no prices.
