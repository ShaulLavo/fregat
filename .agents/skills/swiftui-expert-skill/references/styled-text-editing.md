...
Apply a definition with `attributedTextFormattingDefinition(_:)`. Overloads also accept a bare `AttributeScope` type or a `KeyPath<AttributeScopes, S.Type>` when you only want to limit _which_ attributes survive:
